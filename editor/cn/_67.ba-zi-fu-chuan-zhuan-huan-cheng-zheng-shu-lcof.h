...
public:
    ...
    int solution1(string str) {
      int ans = 0, negative = 0;
      int left = 0, len = str.size();
      // 向左扫描 在遇到 +- 数字之前遇到非空格字符 说明无法转换
      while (left < len && str[left] == ' ') ++left;
      if (left == len) return 0;
      // if (str[left] != '+' && str[left] != '-' && str[left] - '0' < 0 && str[left] - '0' > 9) return 0;  // 非+-非数字
      ...
      // if (str[left] - '0' < 0 || str[left] - '0' > 9) return 0;  // 正负号之后非数字也无效
    ...
    int solution2(string str) {
      int i = 0, flag = 1, len = str.size();
      int res = 0; //默认flag = 1，正数
      while (i < len && str[i] == ' ') ++i;  // str 越界不会溢出
      if (i == len) return 0;
